...
<div style="display: flex; align-items: center; margin-bottom: 20px;">
    <h1 style="margin: 0;"><span class="header-icon"><i class="fa-solid fa-cubes"></i></span> Math Genius</h1>
</div>
    ...
    with st.container():
        ...
        if st.session_state.selected_answer:
            ...
            with st.expander("📖 Detailed Explanation", expanded=True):
                # Convert explanation text to HTML paragraphs
                explanation_steps = [
                    f"<p>{step}</p>" 
                    for step in st.session_state.response_dict["Explanation"].split('\n') 
                    if step.strip()
                ]
                explanation_html = "".join(explanation_steps)
